...
from modeling import ElectraForQuestionAnswering, ElectraForQuestionAnsweringSimple
from optimization import AdamW, get_linear_schedule_with_warmup
from squad_metrics import compute_predictions, squad_evaluate
from squad_utils import squad_convert_examples_to_features, SquadResult, RawResult, SquadV1Processor, SquadV2Processor
...
def _get_layer_depth(n_layers):
    ...
    for layer in range(n_layers):
        key_to_depths["encoder.layer." + str(layer) + "."] = layer + 1
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.layerwise_lr_decay > 0:
        ...
        weight_decay_parameters = [{"params": [], "weight_decay": args.weight_decay,
                                    "lr": args.learning_rate * args.layerwise_lr_decay**(model.num_hidden_layers+2-depth)}
                                   for depth in range(model.num_hidden_layers + 3)]
        for var, p in model.named_parameters():
            update_for_var = False
            for key in layer_depth:
                if key in var:
                    update_for_var = True
                    if any(nd in var for nd in no_decay):
                        no_decay_parameters[layer_depth[key]]["params"].append(p)
                    else:
                        weight_decay_parameters[layer_depth[key]]["params"].append(p)
                    break
            if not update_for_var:
                raise ValueError("No learning rate specified for variable", var)
        optimizer_grouped_parameters = no_decay_parameters + weight_decay_parameters
    else:
...
def main():
    ...
    parser.add_argument(
        "--layerwise_lr_decay", default=0.8, type=float,
    ...
    if args.joint_prediction.lower() == "false":
        args.joint_prediction = False
    else:
        args.joint_prediction = True
    ...
    if args.joint_prediction:
        ...
        model = ElectraForQuestionAnsweringSimple.from_pretrained(
            args.model_name_or_path,
            from_tf=bool(".ckpt" in args.model_name_or_path),
            config=config,
            cache_dir=args.cache_dir if args.cache_dir else None,
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.joint_prediction:
            model = ElectraForQuestionAnswering.from_pretrained(args.output_dir)
        else:
            model = ElectraForQuestionAnsweringSimple.from_pretrained(args.output_dir)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if args.joint_prediction:
                model = ElectraForQuestionAnswering.from_pretrained(checkpoint)
            else:
                model = ElectraForQuestionAnsweringSimple.from_pretrained(checkpoint)
            model.to(args.device)
