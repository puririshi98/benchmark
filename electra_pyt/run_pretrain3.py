...
def train_one_step(config, model, optimizer, scheduler, features, local_step, scaler, clip_norm=1.0):
	...
	if local_step % config.gradient_accumulation_steps == 0:
		...
		#optimizer.zero_grad(set_to_none=True) #Version for latest container
		optimizer.zero_grad()  #If set_to_none doesn't work with your version, comment it out and uncomment this line using the older implementation
		scaler.update()
	else:
		with model.no_sync():
			total_loss, eval_fn_inputs = fwd_bwd(features, scaler, model, config)
...
def main():
    ...
    parser.add_argument("--gpu_affinity", action="store_true", help="Whether to use gpu affinity")
    ...
    # Set GPU affinity
    if args.gpu_affinity:
        print('Use gpu affinity')
        gpu_affinity.set_affinity(args.local_rank, 'socket_unique_interleaved')
                ...
                if local_step % config.gradient_accumulation_steps == 0:
                    ...
                    if config.log_freq > 0 and step != opt_step and (
                        ...
                        print(
                            ...
                            'disc_loss:10.6f}, Gen_acc:{masked_lm_accuracy:4.4f}, '
                            'Disc_acc:{disc_accuracy:4.4}, Perf:{train_perf:4.2f}, Loss Scaler: {loss_scale}, '
                        ...
                        if step == config.num_train_steps:
                            ...
                            logger.info(
                                '<FINAL STEP METRICS> Step:{step:6d}, Loss:{total_loss:10.6f}, Gen_loss:{masked_lm_loss:10.6f}, Disc_loss:{disc_loss:10.6f}, Gen_acc:{masked_lm_accuracy:4.4f}, '
                                'Disc_acc:{disc_accuracy:4.4f}, Perf:{train_perf:4.2f},'.format(
